...
class CombatEngine:
    ...
    def __init__(self, player_lineup: Dict[str, Any], opponent_lineup: Dict[str, Any],
        ...
        from .context import BattleContext as _BC
        _ctx0 = _BC(seed=seed if seed is not None else 0,
                    item_keys=list(self.item_db.keys()))
        self.log = _ctx0.combat_log
        self.ctx = _ctx0
        ...
        # advanceTime（Power of the Moon 等）把疲劳起点提前；在 onPostCombatStart
        # 之后读取一次累计值（CombatTimer.gd:184 timeLeft -= time，≤0 立即 startFatigue）
        self.time_advance = float(getattr(self.ctx.game, "timeAdvance", 0.0) or 0.0)
        self.fatigue_next_tick = FATIGUE_TIME - FATIGUE_START_DELAY - self.time_advance
    ...
    def run(self) -> 'CombatEngine':
        # BattleContext：每场独立的全局状态（无进程级单例，并行/序列化安全）。
        # ropeSpeedups/cubeAdvanced 每场清空（Game.gd:2970）。
        ctx = self.ctx
        ctx.game.ropeSpeedups.clear()
        ctx.game.cubeAdvanced.clear()
        ctx.game.fightEnded = False
        ctx.game.combatLog = self.log
        ctx.time = 0.0
        # 注入全部参战对象（转译代码经 _item.ctx 访问）
        for it in self.player_items + self.opponent_items:
            it.ctx = ctx
        for chx in (self.player, self.opponent):
            chx.ctx = ctx
    ...
    def _fatigue_tick(self, now: float):
        ...
        if not self.fatigue_started:
            ...
            # CombatTimer 节点信号分发（Pumpkin.gd:20 fatigue_start 监听者）
            timer = getattr(self.ctx.game, "combatTimer", None)
            if timer is not None:
                timer.emit_signal("fatigue_start")
        else:
