...
from .behavior import BehaviorRunner
from .stubs import StubRegistry

class BehaviorExecutor(BehaviorRunner):
    def __init__(self, item_key: str = '', spec: dict = None, strict: bool = False):
        super().__init__(item_key, spec or {})

_LEGACY_STUBS = StubRegistry()
class _Noop:
    """遗留视觉桩：链式安全、命中计数可见（不再静默无痕）"""
    def __getattr__(self, name):
        _LEGACY_STUBS.hit(f'legacy_noop.{name}')
        return self
    def __call__(self, *a, **k):
        _LEGACY_STUBS.hit('legacy_noop.__call__')
        return self
...
class Item:
    ...
    def __init__(self, key: str, data: Dict[str, Any],
        ...
        self.ctx = None                              # BattleContext（combat 注入；工具场景默认独立 ctx）
    ...
    def __getattr__(self, name: str):
        ...
        if beh is not None:
            try:
                ...
                pass
        # 登记视觉桩（fail-fast 的显式白名单面：engine/stubs.VISUAL_ITEM_ATTRS）
        from .stubs import VISUAL_ITEM_ATTRS, _LEGACY_STUBS as _LS
        if name in VISUAL_ITEM_ATTRS:
            _LS.hit(f"item_visual.{name}")
            return _LS.make_noop(f"item_visual.{name}")
        raise AttributeError(name)
    ...
    def adjust_cooldown(self) -> float:
        """adjustCooldown — 引擎真值（Item.gd:3805-3812）：
        玩家 cd × randf_range(0.95, 1.05)；对手（低于大师）× randf_range(0.975, 1.05)。
        抖动随机取 ctx.rng（确定性可复现）。"""
        base = self.get_cooldown()
        rng = self.ctx.rng if self.ctx is not None else None
        if rng is None:
            return base
        is_opponent = bool(self.character is not None and not self.character.is_player())
        lo, hi = (0.975, 1.05) if is_opponent else (0.95, 1.05)
    ...
    def behavior(self) -> BehaviorExecutor:
        if self._behavior_executor is None:
            self._behavior_executor = BehaviorExecutor(self.key, self.data.get("behavior"))
    ...
    def _run_item_ready(self):
        ...
        if not self.data.get("behavior"):
            return
        beh = self.behavior
        from .stubs import VISUAL_ITEM_ATTRS as _VIA
        for cls in [None] + beh.extends_chain:
            ...
            for v in iv:
                if _camel_to_snake(v) in engine_managed_attrs():
                    continue
                if v in _VIA or _camel_to_snake(v) in _VIA:
                    # 视觉节点引用（$Icon/TileMap 等）：不填数值默认值，保持缺失
                    # 走 __getattr__ 视觉桩（Leather Bag._ready 访问 .position 等）
                    continue
    ...
    def visual_activate(self, *args, **kwargs):
        """activate()（Item.gd:4686-4737）：发 EventBus "activated" 信号。
        同帧限流：同一物品同帧最多 3 次（4697-4702）。"""
        from types import SimpleNamespace
        now = self.ctx.time if self.ctx is not None else 0.0
        if now > getattr(self, '_last_activation_time', -1.0):
            self._activations_this_frame = 0
            self._last_activation_time = now
        else:
            self._activations_this_frame = getattr(self, '_activations_this_frame', 0) + 1
            if self._activations_this_frame > 3:
                return _Noop()
    ...
    def consume(self, trigger_event=None):
        """consume — 对齐 Item.gd consume(4815)：调用 activate() 发 "activated" 信号。
        游戏中 activate() 会 emitEvent("activated")，触发 Goobert 等信号联动监听者。
        放在 call_behavior("onConsume") 之后——consumed 置 true 后才发信号。"""
        if self.consumed:
            return
        self.consumed = True
        self.call_behavior("onConsume", trigger_event)
        # Item.gd 4815: activate() → emitEvent("activated")（playCombatAni 默认 true）
        self.visual_activate()
    ...
    def _affected_cells_abs(self, color: int = 0) -> list:
        ...
        Affected tile 与 Collision tile 同处一个 CollisionMap 坐标系（extract_grid.py
        decode_tile 出口一致），必须采用与 _init_grid_metadata 相同的归一化基准
        （减去碰撞 min），否则多格物品的受影响格整体错位 (min_x, min_y)。
        """
        from .grid import rotate_cell, cells_from_grid_data
        ...
        rel = grid.get(key) or []
        base = cells_from_grid_data(grid)
        if not base:
            base = [(0, 0)]
        rotated = [rotate_cell(tuple(c), self.grid_rotation) for c in base]
        min_x = min((c[0] for c in rotated), default=0)
        min_y = min((c[1] for c in rotated), default=0)
        cells = set()
        for c in rel:
            rc = rotate_cell(tuple(c), self.grid_rotation)
            # [x, y] 世界坐标（同碰撞归一化基准）-> 背包 (row, col)
            cells.add((self.grid_row + rc[1] - min_y, self.grid_col + rc[0] - min_x))
